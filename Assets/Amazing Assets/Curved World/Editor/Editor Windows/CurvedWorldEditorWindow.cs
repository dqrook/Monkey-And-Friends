...
namespace AmazingAssets
{
    namespace CurvedWorldEditor
    {
        public class CurvedWorldEditorWindow : EditorWindow
        {
            ...
            Texture2D iconForum, iconManual, iconSupport, iconRate, iconMaterial, iconShader, iconSelection;
            ...
            void LoadResources()
            {
                ...
                if(iconForum == null)
                    iconForum = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "forum.png"), typeof(Texture2D));

                if (iconManual == null)
                    iconManual = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "manual.png"), typeof(Texture2D));

                if (iconSupport == null)
                    iconSupport = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "support.png"), typeof(Texture2D));

                if (iconRate == null)
                    iconRate = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "rate.png"), typeof(Texture2D));

                if (iconMaterial == null)
                    iconMaterial = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "material.png"), typeof(Texture2D));

                if (iconShader == null)
                    iconShader = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "shader.png"), typeof(Texture2D));

                if (iconSelection == null)
                    iconSelection = (Texture2D)AssetDatabase.LoadAssetAtPath(Path.Combine(EditorUtilities.GetCurvedWorldEditorFolderPath(), "Editor", "Icons", "selection.png"), typeof(Texture2D));
            }
            ...
            void Draw_AboutTab()
            {
                EditorGUILayout.LabelField("About", guiStyleOptionsHeader, GUILayout.Height(guiStyleOptionsHeaderHeight));
                using (new AmazingAssets.EditorGUIUtility.GUIEnabled(false))
                {
                    EditorGUILayout.LabelField("Version " + EditorUtilities.assetInstalledVersion);
                }
                GUILayout.Space(15);
                ...
                using (new AmazingAssets.EditorGUIUtility.EditorGUILayoutBeginHorizontal())
                {
                    ...
                    using (new AmazingAssets.EditorGUIUtility.GUIEnabled(File.Exists(manualFilePass)))
                    {
                        if (GUILayout.Button(new GUIContent("Manual", iconManual), GUILayout.MaxHeight(24)))
                        { 
                            ...
                            {
                                Debug.LogWarning("File '" + manualFilePass + "' does not exist.");
                            }
                        }
                    }
                    ...
                    if (GUILayout.Button(new GUIContent("Support", iconSupport, EditorUtilities.assetSupportMail + "\nRight click to copy to the clipboard"), GUILayout.MaxHeight(24)))
                    {
                        if (Event.current.button == 1)   //Right click
                        {
                            ...
                            this.ShowNotification(new GUIContent(EditorUtilities.assetSupportMail + "\nCopied to the clipboard"), 3);
                    ...
                    if (GUILayout.Button(new GUIContent("Rate Asset", iconRate), GUILayout.MaxHeight(24)))
                    {
